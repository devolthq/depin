...
export default class DevoltClient {
    ...
    async batteryReport({
        ...
        const tx = await this.program.methods
            ...
            console.log("\n\nEncoded transation: ", tx.instructions[0].data)
            console.log("\n\nTransaction: ", tx)
            ...
            // Decodifique os dados da instrução
            const decodedData = decode(tx.instructions[0].data.toString());
            // const decodedData = deserialize(instructionSchema, 'batteryReport', tx.instructions[0].data);
...
async function runInSequence() {
    ...
    await a.batteryReport({
        ...
        console.log("Battery report 1 successful");
    }
    ).catch((err) => {
        console.log("Error: ", err)
    }).finally(() => {
        console.log("Battery report 1 done");
    });
