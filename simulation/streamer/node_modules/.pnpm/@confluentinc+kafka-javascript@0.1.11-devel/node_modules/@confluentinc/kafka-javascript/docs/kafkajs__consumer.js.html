...
<head>
    <meta charset="utf-8">
    <title>JSDoc: Source: kafkajs/_consumer.js</title>
...
<div id="main">
    ...
    <h1 class="page-title">Source: kafkajs/_consumer.js</h1>
    ...
    <section>
        <article>
            <pre class="prettyprint source linenums"><code>const LibrdKafkaError = require('../error');
...
class Consumer {
   ...
   * @note This should be used only when in KafkaJS compatibility mode (ie. with kafkaJS blocks in the config).
   *       If not in KafkaJS compatibility mode, this is deprecated, use consume() instead as the preferred method.
   */
  async run(config) {
    if (this.#state !== ConsumerState.CONNECTED) {
